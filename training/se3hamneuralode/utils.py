...
def pose_L2_geodesic_loss(s,s_hat, split):
    #################
    x_hat, R_hat, q_dot_hat, _ = torch.split(s_hat, split, dim=2)
    x, R, q_dot, _ = torch.split(s, split, dim=2)
    ...
    return 20*x_loss + 20*vloss + wloss + geo_loss, x_loss, vloss, wloss, geo_loss
