...
from se3hamneuralode import L2_loss, from_pickle, MLP, PSD, SE3HamNODEPX4
...
def get_model():
    ...
    path = 'data/run1/quadrotor-se3ham-rk4-10p3-2500.tar'
...
if __name__ == "__main__":
    ...
    train_x = stats['train_x_cat']
    test_x = stats['test_x_cat']
    t_eval_train = stats['t_eval_train']
    t_eval_test = stats['t_eval_test']
    ...
    traj = 2
    sample_traj = test_x[traj,:,0:12]
    sample_traj_hat = test_x_hat[traj,:,0:12]
    t_eval = 0.004*np.arange(len(sample_traj))
    ...
    #
    # Plot M1^-1(q)
    Dv_net = model.Dv_net(x)
    Dw_net = model.Dw_net(R)
    fig = plt.figure(figsize=figsize)
    ...
    g_pos = g_q.detach().cpu().numpy()[0,0:3,:1]
    g_pos_T = g_pos.T
    g_pos_dagger = np.matmul(np.linalg.inv(np.matmul(g_pos_T, g_pos)), g_pos_T)
    print(g_pos)
    print("\n\n")
    print(g_pos_dagger)
    print("\n\n\n\n")
    g_pos = g_q.detach().cpu().numpy()[0,3:6,1:]
    g_pos_T = g_pos.T
    g_pos_dagger = np.matmul(np.linalg.inv(np.matmul(g_pos_T, g_pos)), g_pos_T)
    print(g_pos)
    print("\n\n")
