...
def train(args):
    ...
    from se3hamneuralode import UnstructuredSE3HamNODE
    from data_collection import get_dataset, arrange_data
    from se3hamneuralode import pose_L2_geodesic_loss, traj_pose_L2_geodesic_loss
    ...
    path = '{}/quadrotor-se3ham-rk4-5p-pre-init-unstructuredHam.tar'.format(args.save_dir)
    model.load_state_dict(torch.load(path, map_location=device))
    ...
    for step in range(0,args.total_steps + 1):
        ...
        if step % args.print_every == 0:
            ...
            label = '-se3ham'
            path = '{}/{}{}-{}-{}p3-unstructuredHam-{}.tar'.format(args.save_dir, args.name, label, args.solver, args.num_points, step)
            torch.save(model.state_dict(), path)
...
if __name__ == "__main__":
    ...
    os.makedirs(args.save_dir) if not os.path.exists(args.save_dir) else None
    label = '-se3ham'
    path = '{}/{}{}-{}-{}p-unstructuredHam.tar'.format(args.save_dir, args.name, label, args.solver, args.num_points)
    torch.save(model.state_dict(), path)
    path = '{}/{}{}-{}-{}p-unstructuredHam-stats.pkl'.format(args.save_dir, args.name, label, args.solver, args.num_points)
