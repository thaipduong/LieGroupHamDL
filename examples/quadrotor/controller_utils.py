...
def plot_states1D(s_traj, s_plan, fig_num=None):
    ...
    fig = plt.figure(fig_num, figsize=(10,7.5))
    ...
    traj_color = "b"
    plan_color = "chocolate"
    plan_style = "dashed"

    ax_px.plot(s_traj[:, -1], s_traj[:, 0], color=traj_color, label = "learned")
    ax_px.plot(s_plan[:, -1], s_plan[:, 0], color= plan_color, linestyle = plan_style, label = "reference")
    ax_px.set_ylabel('x (m)')

    ax_py.plot(s_traj[:, -1], s_traj[:, 1], color=traj_color)
    ax_py.plot(s_plan[:, -1], s_plan[:, 1], color= plan_color, linestyle = plan_style)
    ax_py.set_ylabel('y (m)')

    ax_pz.plot(s_traj[:, -1], s_traj[:, 2], color=traj_color)
    ax_pz.plot(s_plan[:, -1], s_plan[:, 2], color= plan_color, linestyle = plan_style)
    ax_pz.set_ylabel('z (m)')

    ax_vx.plot(s_traj[:, -1], s_traj[:, 3], color=traj_color)
    ax_vx.plot(s_plan[:, -1], s_plan[:, 3], color= plan_color, linestyle = plan_style)
    ax_vx.set_ylabel('x (m/s)')

    ax_vy.plot(s_traj[:, -1], s_traj[:, 4], color=traj_color)
    ax_vy.plot(s_plan[:, -1], s_plan[:, 4], color= plan_color, linestyle = plan_style)
    ax_vy.set_ylabel('y (m/s)')

    ax_vz.plot(s_traj[:, -1], s_traj[:, 5], color=traj_color)
    ax_vz.plot(s_plan[:, -1], s_plan[:, 5], color= plan_color, linestyle = plan_style)
    ax_vz.set_ylabel('z (m/s)')

    ax_yaw.plot(s_traj[:, -1], s_traj[:, 9], color=traj_color)
    ax_yaw.plot(s_plan[:, -1], s_plan[:, 9], color= plan_color, linestyle = plan_style)
    ax_yaw.set_ylabel('yaw (rad)')

    ax_w.plot(s_traj[:, -1], s_traj[:, 10], color=traj_color)
    ax_w.plot(s_traj[:, -1], s_traj[:, 11], color=traj_color)
    ax_w.plot(s_traj[:, -1], s_traj[:, 12], color=traj_color)
    ax_w.plot(s_traj[:, -1], 0*s_traj[:, -1], color= plan_color, linestyle = plan_style)
...
def quadplot_update(s_traj, s_plan, t_curr=None):
    ...
    if len(h_lines) < 2:
        h_ax.plot3D(s_traj[:, 0], s_traj[:, 1], s_traj[:, 2], color = traj_color, label = "learned")
        h_ax.plot3D(s_plan[:, 0], s_plan[:, 1], s_plan[:, 2], '--', color = plan_color, label = "reference")
        h_ax.legend(loc='upper center', ncol=2)
    else:
    ...
    h_ax.set_xlim(s_avg[0] - s_maxrange / 2, s_avg[0] + s_maxrange / 2 - 0.75)
...
def quadplot_comparison(s_gt, s_structuredHam, s_unstructuredHam, s_unstructured, t_curr=None):
    """
    Updates plot designated by an axis handle
    ...
    plt.clf()
    h_ax = plt.axes(projection='3d')
    h_ax.set_xlabel('x (m)', fontsize = 15)
    h_ax.set_ylabel('y (m)', fontsize = 15)
    h_ax.set_zlabel('z (m)', fontsize = 15)
    h_ax.tick_params(labelsize=15)
    # Find min/max position values for each axis to normalize
    s_min = np.nanmin(s_gt[:, 0:3], axis=0)
    s_max = np.nanmax(s_gt[:, 0:3], axis=0)
    s_maxrange = np.max(s_max - s_min) * 1.1 # Add a 10% buffer to edges
    if s_maxrange < 2:
        s_maxrange = 2
    s_avg = (s_max + s_min) / 2
    ...
    h_lines = h_ax.get_lines()
    if len(h_lines) < 4:
        h_ax.plot3D(s_gt[:, 0], s_gt[:, 1], s_gt[:, 2], color = "k", linestyle = "dotted", linewidth=2.5,  label = "ground truth")
        h_ax.plot3D(s_structuredHam[:, 0], s_structuredHam[:, 1], s_structuredHam[:, 2], 'r', linewidth=2.5,  label='structured Hamiltonian')
        h_ax.plot3D(s_unstructuredHam[:, 0], s_unstructuredHam[:, 1], s_unstructuredHam[:, 2], 'g', linewidth=2.5,  label='unstructured Hamiltonian')
        h_ax.plot3D(s_unstructured[:, 0], s_unstructured[:, 1], s_unstructured[:, 2], 'b',linewidth=2.5,  label='black-box')
        h_ax.legend(loc='upper center', ncol=2, fontsize="18")
    else:
        h_lines[0].set_data_3d(s_gt[:, 0], s_gt[:, 1], s_gt[:, 2])
        h_lines[1].set_data_3d(s_structuredHam[:, 0], s_structuredHam[:, 1], s_structuredHam[:, 2])
        h_lines[2].set_data_3d(s_unstructuredHam[:, 0], s_unstructuredHam[:, 1], s_unstructuredHam[:, 2])
        h_lines[3].set_data_3d(s_unstructured[:, 0], s_unstructured[:, 1], s_unstructured[:, 2])

    # Set equalized axis limits
    h_ax.set_xlim(-2.4, -1.4)
    h_ax.set_ylim(1.0, 2.0)
    h_ax.set_zlim(0.3, 1.25)
