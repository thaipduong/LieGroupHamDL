...
from se3hamneuralode import compute_rotation_matrix_from_quaternion, from_pickle, DissipativeSO3HamNODE
...
def get_model():
    model = DissipativeSO3HamNODE(device=device, u_dim=1).to(device)
    path = './data/run1_pend_friction/pendulum-so3ham-rk4-5p.tar'
    model.load_state_dict(torch.load(path, map_location=device))
    path = './data/run1_pend_friction/pendulum-so3ham-rk4-5p-stats.pkl'
...
if __name__ == "__main__":
    ...
    ax.plot(train_loss[0::2], 'b', linewidth=line_width, label='train loss')
    ax.plot(test_loss[0::2], 'r', linewidth=line_width, label='test loss')
    ...
    Dw = model.Dw_net(rotmat)
    V_q = model.V_net(rotmat)
    g_q = model.g_net(rotmat)

    # Plot g(q)
    fig = plt.figure(figsize=figsize)
    plt.plot(theta, np.ones_like(theta), 'k--', label='Ground Truth', color='k', linewidth=line_width)
    plt.plot(theta, beta*g_q.detach().cpu().numpy()[:,0], 'b--', linewidth=line_width, label=r'$\beta B(q)[1]$')
    plt.plot(theta, beta * g_q.detach().cpu().numpy()[:, 1], 'r--', linewidth=line_width, label=r'$\beta B(q)[2]$')
    plt.plot(theta, beta * g_q.detach().cpu().numpy()[:, 2], 'g--', linewidth=line_width, label=r'$\beta B(q)[3]$')
    ...
    # Plot M^-1(q)
    fig = plt.figure(figsize=figsize)
    plt.plot(theta, 0.2 * np.ones_like(theta)/3, label='Ground Truth', color='k', linewidth=line_width-1)
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 2, 2] * beta, 'b--', linewidth=line_width,
             label=r'$D_\omega(q)[3, 3]/\beta$')
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 0, 0] * beta, 'g--', linewidth=line_width,
             label=r'Other $D_\omega(q)[i,j]/\beta$')
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 0, 1] * beta, 'g--', linewidth=line_width)
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 0, 2] * beta, 'g--', linewidth=line_width)
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 1, 0] * beta, 'g--', linewidth=line_width)
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 1, 1] * beta, 'g--', linewidth=line_width)
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 1, 2] * beta, 'g--', linewidth=line_width)
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 2, 0] * beta, 'g--', linewidth=line_width)
    plt.plot(theta, Dw.detach().cpu().numpy()[:, 2, 1] * beta, 'g--', linewidth=line_width)
    plt.xlabel("pendulum angle", fontsize=fontsize_ticks)
    plt.xticks(fontsize=fontsize_ticks)
    plt.yticks(fontsize=fontsize_ticks)
    plt.xlim(-5, 5)
