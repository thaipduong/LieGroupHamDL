...
from se3hamneuralode import from_pickle, DissipativeSO3HamNODE
...
env.reset(ori_rep='rotmat', init_state = np.array([init_angle, u0], dtype=np.float32))
...
for i in range(len(t_eval)-1):
    ...
    u = torch.matmul(gTg_inv_gT, (dH_a - k_d * q_dot + torch.matmul(Dw_q, q_dot)))
    u = u[0]
