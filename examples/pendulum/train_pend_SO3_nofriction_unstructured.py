...
THIS_DIR = os.path.dirname(os.path.abspath(__file__)) + '/data/run3'
...
def train(args):
    ...
    from se3hamneuralode import UnstructuredSO3NODE
    from data_nofriction import get_dataset, arrange_data
    from se3hamneuralode import rotmat_L2_geodesic_loss, traj_rotmat_L2_geodesic_loss


    # Initialize the model
    if args.verbose:
        print("Start UnstructuredSO3NODE training with num of points = {} and solver {}.".format(args.num_points, args.solver))
    ...
    for step in range(args.total_steps + 1):
        ...
        if step % 5000 == 0:
            ...
            label = '-so3ham'
            path = '{}/{}{}-{}-{}p-unstructured-{}.tar'.format(args.save_dir, args.name, label, args.solver, args.num_points, step)
    ...
    train_x = torch.tensor(train_x, requires_grad=True, dtype=float_type).to(device)
    test_x = torch.tensor(test_x, requires_grad=True, dtype=float_type).to(device)
    t_eval = torch.tensor(t_eval, requires_grad=True, dtype=float_type).to(device)
...
if __name__ == "__main__":
    ...
    os.makedirs(args.save_dir) if not os.path.exists(args.save_dir) else None
    label = '-so3ham'
    path = '{}/{}{}-{}-{}p-unstructured.tar'.format(args.save_dir, args.name, label, args.solver, args.num_points)
    torch.save(model.state_dict(), path)
    path = '{}/{}{}-{}-{}p-unstructured-stats.pkl'.format(args.save_dir, args.name, label, args.solver, args.num_points)
