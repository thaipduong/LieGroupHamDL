...
THIS_DIR = os.path.dirname(os.path.abspath(__file__)) + '/data/run2_pend_friction'
...
def train(args):
    ...
    from se3hamneuralode import DissipativeSO3HamNODE
    from data_friction import get_dataset, arrange_data
    ...
    model = DissipativeSO3HamNODE(device=device, u_dim = 1, init_gain=0.5).to(device)
    ...
    data = get_dataset(seed=args.seed, timesteps=20, save_dir=args.save_dir, us=us, ori_rep="rotmat", samples=64, friction=True)
