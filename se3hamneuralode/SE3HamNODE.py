...
class SE3HamNODE(torch.nn.Module):
    def __init__(self, device=None, pretrain = True, M_net1  = None, M_net2 = None, Dv_net = None, Dw_net = None, V_net = None, g_net = None, udim = 4, turnon_dissipation = True):
        ...
        hidden_dim = 400
        if M_net1 is None:
            self.M_net1 = PSD(self.xdim, hidden_dim, self.linveldim, init_gain=init_gain, epsilon = 0.01).to(device)
        else:
            self.M_net1 = M_net1
        if M_net2 is None:
            self.M_net2 = PSD(self.Rdim, hidden_dim, self.twistdim - self.linveldim, init_gain=init_gain, epsilon = 0.01).to(device)
        else:
            self.M_net2 = M_net2

        if self.turnon_dissipation:
            if Dv_net is None:
                self.Dv_net = PSD(self.xdim, hidden_dim, self.linveldim, init_gain=init_gain, epsilon = 0.0).to(device)
            else:
                self.Dv_net = Dv_net
            if Dw_net is None:
                self.Dw_net = PSD(self.Rdim, hidden_dim, self.twistdim - self.linveldim, init_gain=init_gain, epsilon = 0.0).to(device)
            else:
                self.Dw_net = Dw_net
        ...
        if V_net is None:
            ...
            self.V_net = V_net
        if g_net is None:
            self.g_net = MatrixNet(self.posedim, hidden_dim, self.twistdim*self.udim, shape=(self.twistdim,self.udim), init_gain=init_gain).to(device)
        else:
            self.g_net = g_net
        self.device = device
        self.nfe = 0
