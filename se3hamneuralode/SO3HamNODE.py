...
import torch
from se3hamneuralode import MLP, PSD, PSD2, MatrixNet
...
class SO3HamNODE(torch.nn.Module):
    ...
    def __init__(self, M_net = None, V_net = None, g_net = None, device=None, u_dim=3, init_gain=0.01, friction = True):
        super(SO3HamNODE, self).__init__()
        self.rotmatdim = 9
        self.angveldim = 3
        self.friction = friction
        if M_net is None:
            self.M_net = PSD2(self.rotmatdim, 300, self.angveldim, init_gain=init_gain).to(device)
        else:
            self.M_net = M_net
        if V_net is None:
        ...
        if g_net is None:
            if u_dim == 1:
                ...
                self.g_net = MatrixNet(self.rotmatdim, 200, self.angveldim * self.u_dim,
...
class DissipativeSO3HamNODE(torch.nn.Module):
    ...
    def __init__(self, M_net = None, Dw_net = None, V_net = None, g_net = None, device=None, u_dim=3, init_gain=0.01, friction = True):
        super(DissipativeSO3HamNODE, self).__init__()
        self.rotmatdim = 9
        self.angveldim = 3
        self.friction = friction
        if M_net is None:
            self.M_net = PSD(self.rotmatdim, 20, self.angveldim, init_gain=init_gain, epsilon = 0.1).to(device)
        else:
            self.M_net = M_net

        if friction:
            if Dw_net is None:
                self.Dw_net = PSD(self.rotmatdim, 20, self.angveldim, init_gain=init_gain, epsilon = 0.0).to(device)
            else:
                self.Dw_net = Dw_net
        if V_net is None:
            self.V_net = MLP(self.rotmatdim, 20, 1, init_gain=init_gain).to(device)
        else:
            self.V_net = V_net
        ...
        with torch.enable_grad():
            ...
            if self.friction:
                dp = torch.cross(p, dHdp) + torch.cross(q[:, 0:3], dHdq[:, 0:3]) \
                                          + torch.cross(q[:, 3:6], dHdq[:, 3:6]) \
                                          + torch.cross(q[:, 6:9], dHdq[:, 6:9]) \
                                          - torch.squeeze(torch.matmul(Dw_net, torch.unsqueeze(dHdp, dim=2))) \
                                          + F
            else:
                dp = torch.cross(p, dHdp) + torch.cross(q[:, 0:3], dHdq[:, 0:3]) \
                                          + torch.cross(q[:, 3:6], dHdq[:, 3:6]) \
