...
class UnstructuredSE3HamNODE(torch.nn.Module):
    def __init__(self, device=None, pretrain = True, M_net1  = None, M_net2 = None, H_net = None, g_net = None, udim = 4):
        super(UnstructuredSE3HamNODE, self).__init__()
        init_gain = 0.001
        self.xdim = 3
        self.Rdim = 9
        self.linveldim = 3
        ...
        if M_net2 is None:
            ...
            self.M_net2 = M_net2
        if H_net is None:
            self.H_net = MLP(self.posedim + self.twistdim, 400, 1, init_gain=init_gain).to(device)
        else:
            self.H_net = H_net
        if g_net is None:
