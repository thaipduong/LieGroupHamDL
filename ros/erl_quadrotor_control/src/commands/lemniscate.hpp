#ifndef ERL_CONTROL_LEMINSCATE_HPP
#define ERL_CONTROL_LEMINSCATE_HPP
...
namespace ERLControl{
    ...
    class LemniscateCommand : public Command {
        public:
        LemniscateCommand(Eigen::Vector3d aCenter, double aHalfWidth, double aAngularVelocity, double ayaw, double aTime) :
        ...
        void execute() override {
            ...
            target_velocity.x() = -a * omega * (pow(sin(th), 4) + pow(sin(th), 2) + (pow(sin(th), 2) - 1)*pow(cos(th), 2)) / pow(pow(sin(th), 2) + 1, 2);
